...
extension EmojiPickerService {
  ...
  public func systemEmojiAnnotationsAndTagsPublisher(languageCode: String) -> AnyPublisher<[String: String], Error> {
    Future { promise in
  ...
  public func updateUse(emoji: PickerEmoji) -> AnyPublisher<Never, Error> {
    contentDatabase.updateUse(emoji: emoji.name, system: emoji.system)
  }
}

extension EmojiPickerService {
  fileprivate static var maxEmojiVersion: Float = {
