  ...
  init(
    ...
    self.sections = contentDatabase.notificationsPublisher(excludeTypes: excludeTypes)
      ...
      .handleEvents(receiveOutput: {
        guard case .notification(let notification, _, _) = $0.last?.items.last,
...
extension NotificationsService {
  ...
  fileprivate static func groupNotificationSectionItems(_ items: [CollectionItem]) -> [CollectionItem] {
...
private struct GroupableNotification {
  ...
  init?(_ item: CollectionItem) {
    if case .notification(let mastodonNotification, let rules, let statusConfiguration) = item {
