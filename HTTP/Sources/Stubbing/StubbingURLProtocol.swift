  ...
  override public func startLoading() {
    guard
      ...
      let stub = Self.stubsForURLs[url]
        ?? Self.stub(request: request, target: Self.targetsForURLs[url])
    else {
      return
    }
...
extension StubbingURLProtocol {
  public static func setStub(_ stub: HTTPStub, forURL url: URL) {
    stubsForURLs[url] = stub
  }
}

extension StubbingURLProtocol {
  fileprivate class func stub(
    request: URLRequest,
    target: Target? = nil,
    userInfo: [String: Any] = [:]
  ) -> HTTPStub? {
