...
extension ComposeStatusViewController {
  fileprivate func handle(event: ComposeStatusViewModel.Event) {
    switch event {
    case .presentMediaPicker(let compositionViewModel):
      presentMediaPicker(compositionViewModel: compositionViewModel)
    case .presentCamera(let compositionViewModel):
      #if !IS_SHARE_EXTENSION
        presentCamera(compositionViewModel: compositionViewModel)
      #endif
    case .presentDocumentPicker(let compositionViewModel):
      presentDocumentPicker(compositionViewModel: compositionViewModel)
    case .presentEmojiPicker(let tag):
  ...
  fileprivate func apply(postingState: ComposeStatusViewModel.PostingState) {
    switch postingState {
    case .composing:
  ...
  fileprivate func set(compositionViewModels: [CompositionViewModel]) {
    ...
    for removal in diff.removals {
      guard case .remove(_, let id, _) = removal,
  ...
  #if !IS_SHARE_EXTENSION
    fileprivate func presentCamera(compositionViewModel: CompositionViewModel) {
  ...
  fileprivate func presentEmojiPicker(tag: Int) {
  ...
  fileprivate func changeIdentity(_ identity: Identity) {
  ...
  fileprivate func adjustContentInset(notification: Notification) {
  ...
  fileprivate func postActionTitle(
