...
extension TableViewController: NavigationHandling {
  func handle(navigation: Navigation) {
    switch navigation {
    case .collection(let collectionService):
    ...
    case .profile(let profileService):
...
extension TableViewController {
  fileprivate static let bottomInset: CGFloat = .newStatusButtonDimension + .defaultSpacing * 4
  fileprivate static let loadingFooterDebounceInterval: TimeInterval = 0.75

  fileprivate var bottomInset: CGFloat { insetBottom ? Self.bottomInset : 0 }
  ...
  fileprivate func update(_ update: CollectionUpdate) {
    ...
    {
      positionMaintenanceOffset =
        tableView.rectForRow(at: indexPath).origin.y
  ...
  fileprivate func handle(event: CollectionItemEvent) {
    switch event {
    case .ignorableOutput:
      break
    case .toast(let alertItem):
    ...
    case .reload(let collectionItem):
      reload(collectionItem: collectionItem)
    case .presentEmojiPicker(let sourceViewTag, let selectionAction):
  ...
  fileprivate func reload(collectionItem: CollectionItem) {
  ...
  fileprivate func presentEmojiPicker(sourceViewTag: Int, selectionAction: @escaping (String) -> Void) {
  ...
  fileprivate func present(attachmentViewModel: AttachmentViewModel, statusViewModel: StatusViewModel) {
  ...
  fileprivate func present(history: StatusHistoryViewModel) {
  ...
  fileprivate func confirmUnfollow(accountViewModel: AccountViewModel) {
    confirm(
  ...
  fileprivate func confirmHideReblogs(accountViewModel: AccountViewModel) {
    confirm(
      message: String.localizedStringWithFormat(
  ...
  fileprivate func confirmShowReblogs(accountViewModel: AccountViewModel) {
    confirm(
      message: String.localizedStringWithFormat(
  ...
  fileprivate func confirmMute(muteViewModel: MuteViewModel) {
  ...
  fileprivate func confirmUnmute(accountViewModel: AccountViewModel) {
    confirm(
      message: String.localizedStringWithFormat(
  ...
  fileprivate func confirmBlock(accountViewModel: AccountViewModel) {
    let alertController = UIAlertController(
      title: nil,
      message: String.localizedStringWithFormat(
    ...
    let blockAndReportAction = UIAlertAction(
      title: NSLocalizedString("account.block-and-report", comment: ""),
      style: .destructive
    ) { [weak self] _ in
      accountViewModel.block()
  ...
  fileprivate func confirmUnblock(accountViewModel: AccountViewModel) {
    confirm(
      message: String.localizedStringWithFormat(
  ...
  fileprivate func confirmDomainBlock(accountViewModel: AccountViewModel) {
  ...
  fileprivate func confirmDomainUnblock(accountViewModel: AccountViewModel) {
    guard let domain = accountViewModel.domain else { return }
  ...
  fileprivate func accountListEdit(accountViewModel: AccountViewModel, edit: CollectionItemEvent.AccountListEdit) {
    viewModel.applyAccountListEdit(viewModel: accountViewModel, edit: edit)
  }

  fileprivate func editNote(accountViewModel: AccountViewModel) {
    let hostingController = UIHostingController(
  ...
  fileprivate func refreshIfAble() {
    if viewModel.canRefresh {
      viewModel.request(maxId: nil, minId: nil)
    }
  }

  fileprivate func newItemsTapped() {
    scrollToTop(animated: true)
  }

  fileprivate func announceNewItems(newestItem: CollectionItem) {
    switch newestItem {
    case .status:
      switch viewModel.identityContext.appPreferences.statusWord {
      case .toot:
  ...
  fileprivate func hideNewItemsView() {
    showNewItems = false
  }

  fileprivate func showNewItemsChanged(_ show: Bool) {
    if show {
      newItemsView.layoutIfNeeded()

      UIView.animate(
        withDuration: .zeroIfReduceMotion(.defaultAnimationDuration),
        delay: 0,
  ...
  fileprivate func reloadVisibleItems() {
  ...
  fileprivate func setupTimelineActionBarButtonItem(_ timelineActionViewModel: TimelineActionViewModel) {
    switch timelineActionViewModel {
    case .context(let contextTimelineActionViewModel):
    ...
    case .tag(let tagTimelineActionViewModel):
    ...
    case .list(let listTimelineActionViewModel):
      self.navigationItem.rightBarButtonItem = .init(
        title: NSLocalizedString(
    ...
    case .conversations(let conversationsTimelineActionViewModel):
