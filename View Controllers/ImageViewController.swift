...
final class ImageViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    descriptionBackgroundView.isHidden =
      viewModel?.attachment.description == nil
    ...
    contentView.accessibilityLabel =
      viewModel?.attachment.type.accessibilityName
...
extension ImageViewController {
  ...
  private static func write(_ data: Data, description: String, to url: URL) throws {
    guard let src = CGImageSourceCreateWithData(data as CFData, nil),
      let typeIdentifier = CGImageSourceGetType(src)
    else {
...
extension ImageViewController: UIScrollViewDelegate {
  ...
  func scrollViewDidZoom(_ scrollView: UIScrollView) {
      ...
      let contentSize = imageView.image?.size ?? playerView.player?.currentItem?.presentationSize
    {
      ...
      let horizontalInset =
        0.5
        * (newWidth * scrollView.zoomScale > contentView.frame.width
          ? (newWidth - contentView.frame.width)
          : (scrollView.frame.width - scrollView.contentSize.width))
      let verticalInset =
        0.5
        * (newHeight * scrollView.zoomScale > contentView.frame.height
...
extension ImageViewController {
  fileprivate static let maximumZoomScale: CGFloat = 4
  ...
  fileprivate func alertUnableToExportMedia() {
