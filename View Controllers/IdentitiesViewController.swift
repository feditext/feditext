...
final class IdentitiesViewController: UITableViewController {
  ...
  override func tableView(_ tableView: UITableView, shouldHighlightRowAt indexPath: IndexPath) -> Bool {
    if case .identity(let identityViewModel) = dataSource.itemIdentifier(for: indexPath) {
  ...
  override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    case .identity(let identityViewModel):
  ...
  override func tableView(
    ...
    let logOutAction = UIContextualAction(
      style: .destructive,
      title: NSLocalizedString("identities.log-out", comment: "")
    ) { [weak self] _, _, completionHandler in
      guard let self = self, case .identity(let identity) = self.dataSource.itemIdentifier(for: indexPath) else {
