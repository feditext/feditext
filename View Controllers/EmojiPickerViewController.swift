...
final class EmojiPickerViewController: UICollectionViewController {
  ...
  private lazy var dataSource: UICollectionViewDiffableDataSource<PickerEmoji.Category, PickerEmoji> = {
    let cellRegistration = UICollectionView.CellRegistration<EmojiCollectionViewCell, PickerEmoji> { [weak self] in
      guard let self = self else { return }

      $0.viewModel = EmojiViewModel(emoji: $2, identityContext: self.viewModel.identityContext)
    }

    let headerRegistration = UICollectionView.SupplementaryRegistration<EmojiCategoryHeaderView>(
      elementKind: UICollectionView.elementKindSectionHeader
    ) { [weak self] in
      $0.label.text = self?.dataSource.snapshot().sectionIdentifiers[$2.section].displayName
    }

    let dataSource = UICollectionViewDiffableDataSource<PickerEmoji.Category, PickerEmoji>(
  ...
  override func viewDidLoad() {
    ...
    skinToneButton.title =
      viewModel.identityContext.appPreferences.defaultEmojiSkinTone?.example
  ...
  override func collectionView(
    ...
    guard let item = dataSource.itemIdentifier(for: indexPath),
      case .system(let emoji, let infrequentlyUsed) = item,
...
extension EmojiPickerViewController {
  ...
  fileprivate func select(emoji: PickerEmoji) {
