...
extension StatusEndpoint: Endpoint {
  ...
  public var pathComponentsInContext: [String] {
    switch self {
    case .status(let id), .delete(let id), .put(let id, _):
      return [id]
    case .reblog(let id):
      return [id, "reblog"]
    ...
    case .unmute(let id):
      return [id, "unmute"]
    case .react(let id, let name):
      return [id, "react", name]
    case .unreact(let id, let name):
      return [id, "unreact", name]
    case .pleromaReact(let id, let name), .pleromaUnreact(let id, let name):
  ...
  public var jsonBody: [String: Any]? {
    switch self {
    case .post(let components), .put(_, let components):
  ...
  public var requires: APICapabilityRequirements? {
    ...
    case .post(let components):
      // These are *future* GtS features and have some surprising behavior in 0.16.
      if components.visibility == .mutualsOnly || components.federated != nil || components.replyable != nil
        || components.boostable != nil || components.likeable != nil
      {
        return [:]
      }
  ...
  public var notFound: EntityNotFound? {
    ...
    case .status(let id),
      ...
      .react(let id, _),
      .unreact(let id, _),
      .pleromaReact(let id, _),
      .pleromaUnreact(let id, _):
      return .status(id)
    }
  }
}
