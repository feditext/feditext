...
final class TouchFallthroughTextView: UITextView, EmojiInsertable {
  ...
  override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
    ...
    guard let touch = touches.first,
      let (_, rect) = urlAndRect(at: touch.location(in: self))
    else {
      return
    }
...
extension TouchFallthroughTextView {
  fileprivate static let linkHighlightViewTransform = CGAffineTransform(scaleX: 1.1, y: 1.1)

  fileprivate func removeLinkHighlightView() {
  ...
  fileprivate static func backgroundColor(for quoteLevel: Int) -> UIColor {
    return .init { traitCollection in
      ...
      b +=
        (traitCollection.userInterfaceStyle == .light ? -1.0 : 1.0)
  ...
  fileprivate struct Blockquote {
    var rects: [CGRect]
  }

  fileprivate func updateBlockquotesLayer() {
  ...
  fileprivate func updateHorizontalRuleLayerSiren(_ zPosition: CGFloat) {
    attributedText.enumerateAttribute(
