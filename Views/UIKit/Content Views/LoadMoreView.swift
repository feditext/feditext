...
extension LoadMoreView {
  fileprivate static let directionChangeMax = CGFloat.pi
  fileprivate static let directionChangeIncrement = CGFloat.pi / 10
  ...
  fileprivate func initialSetup() {
    ...
    accessibilityCustomActions = [
      UIAccessibilityCustomAction(
        name: aboveAccessibilityActionName
      ) { [weak self] _ in
      ...
      UIAccessibilityCustomAction(
        name: belowAccessibilityActionName
      ) { [weak self] _ in
        self?.directionChange = Self.directionChangeMax
        self?.updateDirectionChange(animated: false)
        NotificationCenter.default.post(name: Self.accessibilityCustomAction, object: self)
  ...
  fileprivate func applyLoadMoreConfiguration() {
  ...
  fileprivate func updateDirectionChange(animated: Bool) {
    if animated {
  ...
  fileprivate func performDirectionChangeUpdates() {
