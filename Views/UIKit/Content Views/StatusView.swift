...
extension StatusView {
  fileprivate static let actionButtonTitleEdgeInsets = UIEdgeInsets(top: 0, left: 2, bottom: 0, right: 0)
  fileprivate static let reblogAvatarDimension: CGFloat = .avatarDimension * 7 / 8
  ...
  fileprivate static let replyButtonAccessibilityLabel = NSLocalizedString(
    "status.reply-button.accessibility-label",
    comment: ""
  )

  fileprivate static let shareButtonAccessibilityLabel = NSLocalizedString(
    "status.share-button.accessibility-label",
    comment: ""
  )

  fileprivate static func reblogButtonAccessibilityLabel(reblogged: Bool) -> String {
    if reblogged {
  ...
  fileprivate static func favoriteButtonAccessibilityLabel(favorited: Bool) -> String {
    if favorited {
  ...
  fileprivate func applyStatusConfiguration() {
  ...
  fileprivate func menu(viewModel: StatusViewModel) -> UIMenu {
    var sections = [UIMenu]()
    ...
    if viewModel.canEditReactions && viewModel.canAddMoreReactions {
      firstSectionItems.append(
        UIAction(
          ...
          image: UIImage(systemName: "face.smiling.inverse")
        ) { [weak self] _ in
    ...
    if viewModel.canToggleMute {
      secondSectionItems.append(
        UIAction(
          ...
          image: UIImage(systemName: viewModel.muted ? "speaker.wave.2" : "speaker.slash")
        ) { _ in
  ...
  fileprivate func setButtonImages(font: UIFont) {
  ...
  @objc fileprivate func reblogButtonDoubleTap(sender: UIButton, event: UIEvent) {
    guard
      ...
      event.allTouches?.first?.tapCount == 2
    else {
      return
    }
  ...
  @objc fileprivate func favoriteButtonDoubleTap(sender: UIButton, event: UIEvent) {
    guard
  ...
  fileprivate func setReblogButtonColor(reblogged: Bool) {
  ...
  fileprivate func setFavoriteButtonColor(favorited: Bool) {
  ...
  fileprivate func configureUserInteractionEnabledForAccessibility() {
  ...
  fileprivate func accessibilityCustomActions(viewModel: StatusViewModel) -> [UIAccessibilityCustomAction] {
    ...
    if replyButton.isEnabled {
      actions.append(
        UIAccessibilityCustomAction(
          name: Self.replyButtonAccessibilityLabel
        ) { _ in
          viewModel.reply()
    ...
    if viewModel.canBeReblogged, reblogButton.isEnabled {
      ...
      actions.append(
        UIAccessibilityCustomAction(
          name: Self.reblogButtonAccessibilityLabel(reblogged: reblogged)
        ) { [weak self] _ in
          self?.reblog()
    ...
    if favoriteButton.isEnabled {
      ...
      actions.append(
        UIAccessibilityCustomAction(
          name: Self.favoriteButtonAccessibilityLabel(favorited: favorited)
        ) { [weak self] _ in
          self?.favorite()
    ...
    if shareButton.isEnabled {
      actions.append(
        UIAccessibilityCustomAction(
          name: Self.shareButtonAccessibilityLabel
        ) { _ in
    ...
    if menuButton.isEnabled {
      ...
      if viewModel.isMine {
        actions += [
          UIAccessibilityCustomAction(
            name: NSLocalizedString("status.delete", comment: "")
          ) { _ in
            viewModel.confirmDelete(redraft: false)
          ...
          UIAccessibilityCustomAction(
            name: NSLocalizedString("status.delete-and-redraft", comment: "")
          ) { _ in
            viewModel.confirmDelete(redraft: true)
        ...
        actions.append(
          UIAccessibilityCustomAction(
            name: NSLocalizedString("report", comment: "")
          ) { _ in
            viewModel.reportStatus()
  ...
  fileprivate func authenticatedIdentitiesMenu(action: @escaping (Identity) -> Void) -> UIMenu {
...
extension UIButton {
  fileprivate func setCountTitle(count: Int, isContextParent: Bool) {
