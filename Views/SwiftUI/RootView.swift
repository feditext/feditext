  ...
  var body: some View {
    if let navigationViewModel = viewModel.navigationViewModel {
      MainNavigationView { navigationViewModel }
        ...
        .onReceive(
          navigationViewModel.identityContext.$appPreferences.map(\.colorScheme),
...
extension RootView {
  fileprivate func setColorScheme(_ colorScheme: AppPreferences.ColorScheme) {
  ...
  fileprivate func setTintColor(_ tintColor: Identity.Preferences.TintColor?) {
    for scene in UIApplication.shared.connectedScenes.compactMap({ $0 as? UIWindowScene }) {
      for window in scene.windows {
