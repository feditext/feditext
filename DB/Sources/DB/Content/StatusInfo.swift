...
extension StatusInfo {
  fileprivate static func addingOptionalIncludes<T: DerivableRequest>(
    _ request: T,
    _ filterContext: Filter.Context?
  ) -> T where T.RowDecoder == StatusRecord {
    var request =
      ...
      .including(
        optional: RelatedStatusInfo.addingIncludes(StatusRecord.quote, filterContext).forKey(CodingKeys.quoteInfo)
      )
      .including(
        optional: RelatedStatusInfo.addingIncludes(StatusRecord.reblog, filterContext).forKey(CodingKeys.reblogInfo)
      )
