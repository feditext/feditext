...
extension IdentityDatabase {
  ...
  public func deleteIdentity(id: Identity.Id) -> AnyPublisher<Never, Error> {
    databaseWriter.mutatingPublisher(updates: IdentityRecord.filter(IdentityRecord.Columns.id == id).deleteAll)
  }

  public func updateLastUsedAt(id: Identity.Id) -> AnyPublisher<Never, Error> {
    databaseWriter.mutatingPublisher {
      try IdentityRecord
  ...
  public func updateInstance(_ instance: Instance, id: Identity.Id) -> AnyPublisher<Never, Error> {
    databaseWriter.mutatingPublisher {
  ...
  public func updateAccount(_ account: Account, id: Identity.Id) -> AnyPublisher<Never, Error> {
    databaseWriter.mutatingPublisher {
  ...
  public func confirmIdentity(id: Identity.Id) -> AnyPublisher<Never, Error> {
    databaseWriter.mutatingPublisher {
      try IdentityRecord
        .filter(IdentityRecord.Columns.id == id)
  ...
  public func updatePreferences(_ preferences: Mastodon.Preferences, id: Identity.Id) -> AnyPublisher<Never, Error> {
    databaseWriter.mutatingPublisher {
      guard
        let storedPreferences = try IdentityRecord.filter(IdentityRecord.Columns.id == id)
          .fetchOne($0)?
          .preferences
      else {
  ...
  public func updatePreferences(_ preferences: Identity.Preferences, id: Identity.Id) -> AnyPublisher<Never, Error> {
  ...
  public func identityPublisher(id: Identity.Id, immediate: Bool) -> AnyPublisher<Identity, Error> {
  ...
  public func recentIdentitiesPublisher(excluding: Identity.Id) -> AnyPublisher<[Identity], Error> {
    ValueObservation.tracking(
  ...
  public func authenticatedIdentitiesPublisher(excluding: Identity.Id) -> AnyPublisher<[Identity], Error> {
    ValueObservation.tracking(
      IdentityInfo.request(IdentityRecord.order(IdentityRecord.Columns.lastUsedAt.desc))
        .filter(
  ...
  public func immediateMostRecentlyUsedIdentityIdPublisher() -> AnyPublisher<Identity.Id?, Error> {
    ValueObservation.tracking(
  ...
  public func fetchIdentitiesWithOutdatedDeviceTokens(deviceToken: Data) -> AnyPublisher<[Identity], Error> {
  ...
  public func mostRecentAuthenticatedIdentity() throws -> Identity? {
    guard
      let info = try databaseWriter.read(
        IdentityInfo.request(IdentityRecord.order(IdentityRecord.Columns.lastUsedAt.desc))
          .filter(
            IdentityRecord.Columns.authenticated == true
              && IdentityRecord.Columns.pending == false
