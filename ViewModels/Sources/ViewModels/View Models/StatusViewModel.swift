...
public final class StatusViewModel: AttachmentsRenderingViewModel, ObservableObject {
  ...
  init(
    ...
    rebloggedByDisplayName =
      statusService.status.account.displayName.isEmpty
...
extension StatusViewModel {
  ...
  public func presentDisplayStatus() {
    eventsSubject.send(
      Just(
        .navigation(
          .collection(
  ...
  public var isMine: Bool { statusService.status.displayStatus.account.id == identityContext.identity.account?.id }
  ...
  public var showContentToggled: Bool {
  ...
  public var alwaysExpandSpoilers: Bool {
    identityContext.identity.preferences.readingExpandSpoilers
  }

  public var shouldHideDueToSpoiler: Bool {
  ...
  public var foldLongContent: Bool {
    identityContext.appPreferences.foldLongPosts
  }

  public var hasLongContent: Bool {
  ...
  public var shouldHideDueToLongContent: Bool {
    foldLongContent && hasLongContent
  }

  public var shouldShowContentWarningButton: Bool {
  ...
  public var shouldShowContentPreview: Bool {
  ...
  public var accountName: String { "@".appending(statusService.status.displayStatus.account.acct) }
  ...
  public var rebloggerAvatarURL: URL? {
    if identityContext.appPreferences.animateAvatars == .everywhere {
  ...
  public var time: String? { statusService.status.displayStatus.lastModified.timeAgo }

  public var accessibilityTime: String? { statusService.status.displayStatus.lastModified.accessibilityTimeAgo }

  public var canViewEditHistory: Bool {
    StatusEditsEndpoint.history(id: "").canCallWith(identityContext.apiCapabilities)
  }

  public var edited: Bool { statusService.status.displayStatus.edited }

  public var contextParentTime: String {
    Self.contextParentDateFormatter.string(from: statusService.status.displayStatus.createdAt)
  }

  public var contextParentEditedTime: String? {
    statusService.status.displayStatus.editedAt.map { Self.contextParentDateFormatter.string(from: $0) }
  }

  public var accessibilityContextParentTime: String {
    Self.contextParentAccessibilityDateFormatter.string(from: statusService.status.displayStatus.createdAt)
  }

  public var accessibilityContextParentEditedTime: String? {
    statusService.status.displayStatus.editedAt
      .map { Self.contextParentAccessibilityDateFormatter.string(from: $0) }
  }

  public var applicationName: String? { statusService.status.displayStatus.application?.name }
  ...
  public var mentions: [Mention] { statusService.status.displayStatus.mentions }

  public var visibility: Status.Visibility { statusService.status.displayStatus.visibility }

  public var repliesCount: Int { statusService.status.displayStatus.repliesCount }

  public var reblogsCount: Int { statusService.status.displayStatus.reblogsCount }

  public var favoritesCount: Int { statusService.status.displayStatus.favouritesCount }

  public var reblogged: Bool { statusService.status.displayStatus.reblogged }

  public var favorited: Bool { statusService.status.displayStatus.favourited }

  public var bookmarked: Bool { statusService.status.displayStatus.bookmarked }

  public var sensitive: Bool { statusService.status.displayStatus.sensitive }

  public var pinned: Bool? { statusService.status.displayStatus.pinned }

  public var muted: Bool { statusService.status.displayStatus.muted }

  /// This post was filtered by the server. See ``shouldFilter`` for whether we should hide it on the client.
  public var filtered: Bool { !statusService.status.displayStatus.filtered.isEmpty }
  ...
  public var isPollExpired: Bool { statusService.status.displayStatus.poll?.expired ?? true }

  public var hasVotedInPoll: Bool { statusService.status.displayStatus.poll?.voted ?? false }

  public var isPollMultipleSelection: Bool { statusService.status.displayStatus.poll?.multiple ?? false }

  public var pollOptions: [Poll.Option] { statusService.status.displayStatus.poll?.options ?? [] }

  public var pollVotersCount: Int {
    guard let poll = statusService.status.displayStatus.poll else { return 0 }

    return poll.votersCount ?? poll.votesCount
  }

  public var pollOwnVotes: Set<Int> { Set(statusService.status.displayStatus.poll?.ownVotes ?? []) }

  public var pollTimeLeft: String? {
  ...
  public var canBeReblogged: Bool {
  ...
  public var tagViewModels: [TagViewModel] {
  ...
  public func toggleReblogged(identityId: Identity.Id? = nil) {
    eventsSubject.send(
  ...
  /// Emoji reactions.
  public var reactions: [Reaction] { statusService.status.displayStatus.unifiedReactions }

  public var canEditReactions: Bool { statusService.canEditReactions }

  /// Some instances have a cap on the number of reactions you can use.
  public var canAddMoreReactions: Bool {
  ...
  public var reasonTagIDs: Set<TagViewModel.ID> {
  ...
  public typealias TagPair = (id: Tag.ID, name: String)

  /// Find any hashtags that are attached to the status but don't appear in the text.
  public var outOfTextTags: [TagPair] {
  ...
  public var splitTrailingHashtags: (AttributedString.Index, [TagPair]) {
...
extension StatusViewModel {
  ...
  fileprivate enum ComposeOperation {
    case redraft(Status)
    case edit(Status)
  ...
  fileprivate func compose(_ operationPublisher: AnyPublisher<ComposeOperation, Error>) {
    let identityContext = identityContext
    let isContextParent = configuration.isContextParent
    let timeline = timeline
    let followedTags = followedTags
  ...
  fileprivate func handleToasts(_ error: Error) -> AnyPublisher<CollectionItemEvent, Error> {
