...
public class CollectionItemsViewModel: ObservableObject {
          ...
          let statusIds = Set(
            self.lastUpdate.sections
              ...
              .compactMap { item -> Status.Id? in
                guard case .status(let status, _, _, _) = item else { return nil }
  ...
  public func requestNextPage(fromIndexPath indexPath: IndexPath) {
    guard
      let maxId = collectionService.preferLastPresentIdOverNextPageMaxId
  ...
  public func viewModel(indexPath: IndexPath) -> Any? {
    ...
    switch item {
    case .status(let status, let configuration, let authorRelationship, let rebloggerRelationship):
    ...
    case .notification(let notification, let rules, let statusConfiguration):
      let viewModel: Any
    ...
    case .multiNotification(let notifications, let notificationType, let date, let status):
      if let cachedViewModel = cachedViewModel {
        return cachedViewModel
      }
    ...
    case .moreResults(let moreResults):
      if let cachedViewModel = cachedViewModel {
        return cachedViewModel
      }
...
extension CollectionItemsViewModel: CollectionViewModel {
  ...
  public func select(indexPath: IndexPath) {
    ...
    case .account(let account, _, let relationship, let familiarFollowers, _):
  ...
  public func canSelect(indexPath: IndexPath) -> Bool {
    ...
    case .status(_, let configuration, _, _):
...
extension CollectionItemsViewModel {
  ...
  fileprivate func realMaxId(maxId: String?) -> String? {
  ...
  fileprivate func idForScrollPositionMaintenance(newSections: [CollectionSection]) -> CollectionItem.Id? {
    ...
    if collectionService is ContextService,
      ...
      let contextParent = newItems.first(where: {
        guard case .status(_, let configuration, _, _) = $0 else { return false }
      ...
      })
    {
      ...
      if let lastSelectedLoadMore = lastSelectedLoadMore {
        for removal in difference.removals {
          if case .remove(_, let item, _) = removal,
            case .loadMore(let loadMore) = item,
