...
extension EditFilterViewModel {
  public var isNew: Bool { filter.id == Filter.newFilterId }

  public var isSaveDisabled: Bool { filter.phrase.isEmpty || filter.context.isEmpty }

  /// Some backends don't support irreversible/hide filters yet.
  public var canUseIrreversible: Bool {
    FilterEndpoint
      .create(phrase: "", context: [], irreversible: true, wholeWord: false, expiresIn: nil)
      .canCallWith(identityContext.apiCapabilities)
  }

  public func toggleSelection(context: Filter.Context) {
