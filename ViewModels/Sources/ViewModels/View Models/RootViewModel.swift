...
public final class RootViewModel: ObservableObject {
  ...
  public init(
    ...
    registerForRemoteNotifications: @escaping () -> AnyPublisher<Data, Error>
  ) throws {
...
extension RootViewModel {
  public func identitySelected(id: Identity.Id?) {
    identitySelected(id: id, immediate: false, notify: false)
  }

  public func deleteIdentity(id: Identity.Id) {
  ...
  public func clearNavigationCache() {
    NavigationService.clearCache()
  }
}

extension RootViewModel {
  fileprivate static let identityChangeNotificationUserInfoKey =
    "com.metabolist.metatext.identity-change-notification-user-info-key"
  fileprivate static let removeIdentityChangeNotificationAfter = DispatchTimeInterval.seconds(10)

  fileprivate func identitySelected(id: Identity.Id?, immediate: Bool, notify: Bool) {
    navigationViewModel?.presentingSecondaryNavigation = false

    guard
      let id = id,
      let identityService = try? allIdentitiesService.identityService(id: id)
    else {
  ...
  fileprivate func handle(event: UserNotificationService.Event) {
    switch event {
    case .willPresentNotification(let notification, let completionHandler):
    ...
    case .didReceiveResponse(let response, let completionHandler):
  ...
  fileprivate func handle(pushNotification: PushNotification, identityId: Identity.Id) {
  ...
  fileprivate func notifyIdentityChange(identityContext: IdentityContext) {
