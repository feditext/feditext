...
public final class ExploreViewModel: ObservableObject {
  ...
  init(service: ExploreService, identityContext: IdentityContext) {
    ...
    searchViewModel = SearchViewModel(identityContext: identityContext, .exploreTab)
    ...
    statusEventsSubject
      ...
      .compactMap {
        switch $0 {
        case CollectionItemEvent.navigation(let nav):
...
extension ExploreViewModel {
  ...
  public func viewModel(status: Status) -> StatusViewModel {
    .init(
  ...
  public func select(item: ExploreViewModel.Item) {
    switch item {
    case .tag(let tag):
      eventsSubject.send(
        .navigation(
