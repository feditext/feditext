...
extension Secrets {
  public static func identityDatabaseName(keychain: Keychain.Type) throws -> String {
    do {
  ...
  public static func imageCacheKey(keychain: Keychain.Type) throws -> Data {
    do {
  ...
  public static func databaseKey(identityId: UUID?, keychain: Keychain.Type) throws -> String {
    ...
    do {
      passphraseData =
        try scopedSecrets?.item(.databaseKey)
  ...
  public func getInstanceURL() throws -> URL {
    try item(.instanceURL)
  }

  public func setInstanceURL(_ instanceURL: URL) throws {
  ...
  public func setClientSecret(_ clientSecret: String) throws {
  ...
  public func setAccessToken(_ accessToken: String) throws {
  ...
  public func setAccountId(_ accountId: String) throws {
  ...
  public func getSoftwareName() throws -> String {
    try item(.softwareName)
  }

  public func setSoftwareName(_ name: String) throws {
    try set(name, forItem: .softwareName)
  }

  public func getSoftwareVersion() throws -> String {
    try item(.softwareVersion)
  }

  public func setSoftwareVersion(_ version: String) throws {
    try set(version, forItem: .softwareVersion)
  }

  public func getAPIFeaturesRawValues() throws -> [String] {
    let rawValues: String = try item(.apiFeaturesRawValues)
    return rawValues.split(separator: ",").map { String($0) }
  }

  public func setAPIFeaturesRawValues(_ rawValues: [String]) throws {
    try set(rawValues.joined(separator: ","), forItem: .apiFeaturesRawValues)
  }

  public func getAPICompatibilityModeRawValue() throws -> String {
    try item(.apiCompatibilityModeRawValue)
  }

  public func setAPICompatibilityModeRawValue(_ rawValue: String) throws {
    try set(rawValue, forItem: .apiCompatibilityModeRawValue)
  }
  ...
  public func getPushAuth() throws -> Data? {
    try item(.pushAuth)
  }
}

extension Secrets {
  fileprivate static let keychainServiceName = "com.metabolist.metatext"
  fileprivate static let databaseKeyLength = 48
  ...
  fileprivate static func setUnscoped(_ data: SecretsStorable, forItem item: Item, keychain: Keychain.Type) throws {
    try set(data, forAccount: item.rawValue, keychain: keychain)
  }

  fileprivate static func unscopedItem<T: SecretsStorable>(_ item: Item, keychain: Keychain.Type) throws -> T {
    try get(account: item.rawValue, keychain: keychain)
  }

  fileprivate func scopedKey(item: Item) -> String {
    identityId.uuidString.appending(".").appending(item.rawValue)
  }

  fileprivate func set(_ data: SecretsStorable, forItem item: Item) throws {
    try Self.set(data, forAccount: scopedKey(item: item), keychain: keychain)
  }

  fileprivate func item<T: SecretsStorable>(_ item: Item) throws -> T {
